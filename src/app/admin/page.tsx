  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 lg:px-6 py-12 animate-in fade-in duration-700">
      ...
      <div className="mb-12">
        ...
        <p className="text-lg text-dark-secondary-text max-w-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        {[
          { icon: CheckCircle, label: 'Pending Approvals', value: stats.pendingGuides, color: 'text-orange-400', bg: 'bg-orange-400/10' },
          { icon: Flag, label: 'Flagged Content', value: stats.flagged, color: 'text-red-400', bg: 'bg-red-400/10' },
          { icon: Users, label: 'Active Travelers', value: stats.users.toLocaleString(), color: 'text-blue-400', bg: 'bg-blue-400/10' },
        ].map((stat, i) => (
          <div key={i} className="relative group p-8 rounded-[32px] bg-dark-surface border border-dark-border hover:border-brand-ember/30 transition-all duration-300">
            ...
            <h3 className="text-4xl font-bold text-dark-primary-text mb-1">
              {loadingStats ? <div className="h-10 w-16 bg-white/5 rounded animate-pulse" /> : stat.value}
            </h3>
            <p className="text-sm font-semibold text-dark-secondary-text uppercase tracking-widest">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="rounded-[32px] bg-dark-surface border border-dark-border p-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button
                ...
                className="group p-6 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all text-left flex flex-col gap-3"
              >
                ...
                <div>
                  <h4 className="font-bold">Reset & Seed</h4>
                  <p className="text-xs text-dark-secondary-text mt-1">Populate platform with fresh initial data.</p>
                </div>
              </button>
              ...
              <button
                ...
                className="group p-6 rounded-2xl bg-brand-ember/10 border border-brand-ember/20 hover:bg-brand-ember/20 transition-all text-left flex flex-col gap-3"
              >
            ...
            {statusMessage && (
              <div className="mt-8 p-4 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3 animate-in slide-in-from-bottom-2">
                <div className="w-2 h-2 rounded-full bg-brand-ember animate-pulse" />
                <p className="text-sm font-medium text-dark-primary-text">{statusMessage}</p>
              </div>
            )}
          </div>

          <div className="rounded-[32px] bg-dark-surface border border-dark-border overflow-hidden">
            <BucketTest />
          </div>
        </div>

        {/* Sidebar Log/Info */}
        <div className="rounded-[32px] bg-dark-surface border border-dark-border p-8 flex flex-col justify-between">
          <div>
            <h4 className="text-lg font-bold mb-4 uppercase tracking-widest text-dark-secondary-text opacity-50">System Logs</h4>
            <div className="space-y-4 font-mono text-[10px]">
              <div className="flex gap-3 text-green-400/60">
                <span>[19:30:12]</span>
          ...
          <div className="mt-8 p-6 rounded-2xl bg-white/5 border border-white/10">
            <p className="text-xs text-dark-secondary-text leading-relaxed italic">
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="max-w-md w-full bg-dark-surface border border-dark-border rounded-[40px] p-10 shadow-2xl relative overflow-hidden">
            ...
            <h3 className="text-3xl font-bold mb-4">Execute Sync?</h3>
            <p className="text-dark-secondary-text mb-10 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button 
                ...
                className="flex-1 px-8 py-4 rounded-2xl bg-white/5 border border-white/10 font-bold hover:bg-white/10 transition-all"
              >
                Cancel
              </button>
            </div>
